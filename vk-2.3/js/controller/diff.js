 ...
 */
'use strict';
app.controller('DiffCtrl', function ($scope, ngToast, $timeout, cfpLoadingBar) {
            ...
            if (data.response) {
                ...
                } else // если конец то
                {
                    if (_arrMutual.length <= 0)
                        ...
                        _arrMutual = ArrMath.Diff(_arrMutualMemberGroupIDs, _arrMutual);
                    $scope.$digest();
            ...
            VK.api('friends.get', {user_id: $scope.arrUrls[index].id, v: '3.0', https: 1}, function (data) {
                if (data.response) {
                    if (_arrMutual.length <= 0)
                        ...
                        _arrMutual = ArrMath.Diff(data.response, _arrMutual);
                    nextFuncMutual(index);
                }
            });
        }
